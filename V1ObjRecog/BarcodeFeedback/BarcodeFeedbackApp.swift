...
class BarcodeScannerv2: NSObject, ObservableObject {
    ...
    func addOverlay(for barcodes: [VNBarcodeObservation], on overlayLayer: CALayer) {
        DispatchQueue.main.async {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        let request = VNDetectBarcodesRequest { request, error in
            if let results = request.results as? [VNBarcodeObservation] {
                DispatchQueue.main.async {
                    self.addOverlay(for: results, on: self.overlayLayerv2 ?? CALayer())
...
struct CameraViewv2: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        scanner.overlayLayerv2 = overlayLayer
    ...
    func updateUIView(_ uiView: UIView, context: Context) {
        ...
        DispatchQueue.main.async {
            if let previewLayer = uiView.layer.sublayers?.first(where: { $0 is AVCaptureVideoPreviewLayer }) as? AVCaptureVideoPreviewLayer {
                ...
                scanner.overlayLayerv2?.frame = uiView.bounds
                print("CameraViewv2: Updated preview layer and overlay layer frames")
            }
        }
